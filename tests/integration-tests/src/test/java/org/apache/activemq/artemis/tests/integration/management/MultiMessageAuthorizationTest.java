...
import org.apache.activemq.artemis.core.server.plugin.impl.BrokerMessageMultiAuthorizationPlugin;
...
public class MultiMessageAuthorizationTest extends ActiveMQTestBase {
   ...
   public void setUp() throws Exception {
      ...
      BrokerMessageMultiAuthorizationPlugin plugin = new BrokerMessageMultiAuthorizationPlugin();
      plugin.init(Collections.singletonMap("ROLES", "a, b, programmers"));
   ...
   @Test
   public void testMessageAuthorizationQueueMulti() throws Exception {
      JmsConnectionFactory factory = new JmsConnectionFactory("amqp://127.0.0.1:61616");
      Connection connection = factory.createConnection("first", "secret");
      Session session = connection.createSession(false, Session.AUTO_ACKNOWLEDGE);
      javax.jms.Queue queue = session.createQueue("TestQueueNotAuth");
      ...
      TextMessage bMessage = session.createTextMessage();
      bMessage.setStringProperty("requiredRole", "b,a,c");
      Connection aConnection = factory.createConnection("a", "a");
      Session aSession = aConnection.createSession(false, Session.AUTO_ACKNOWLEDGE);
      aConnection.start();
      MessageConsumer aConsumer = aSession.createConsumer(queue);
      ...
      Message aMsg = aConsumer.receiveNoWait();
      Assert.assertNotNull(aMsg);
      Assert.assertEquals("b,a,c", aMsg.getStringProperty("requiredRole"));

      aConnection.close();
   }

   @Test
   public void testMessageAuthorizationQueueMultiNotAuthorized() throws Exception {
      JmsConnectionFactory factory = new JmsConnectionFactory("amqp://127.0.0.1:61616");
      Connection connection = factory.createConnection("first", "secret");
      Session session = connection.createSession(false, Session.AUTO_ACKNOWLEDGE);
      javax.jms.Queue queue = session.createQueue("TestQueueNotAuth");
      ...
      TextMessage bMessage = session.createTextMessage();
      bMessage.setStringProperty("requiredRole", "b,c");
      Connection aConnection = factory.createConnection("a", "a");
      Session aSession = aConnection.createSession(false, Session.AUTO_ACKNOWLEDGE);
      aConnection.start();
      MessageConsumer aConsumer = aSession.createConsumer(queue);
   ...
   @Test
   public void testMessageAuthorizationTopicMulti() throws Exception {
      JmsConnectionFactory factory = new JmsConnectionFactory("amqp://127.0.0.1:61616");
      Connection connection = factory.createConnection("first", "secret");
      Session session = connection.createSession(false, Session.AUTO_ACKNOWLEDGE);
      javax.jms.Topic topic = session.createTopic(TOPIC.toString());
      MessageProducer producer = session.createProducer(topic);
      TextMessage aMessage = session.createTextMessage();
      aMessage.setStringProperty("requiredRole", "a,programmers");
      TextMessage bMessage = session.createTextMessage();
      bMessage.setStringProperty("requiredRole", "b,programmers");
      ...
      Connection fConnection = factory.createConnection("first", "secret");
      Session fSession = fConnection.createSession(false, Session.AUTO_ACKNOWLEDGE);
      fConnection.start();

      MessageConsumer aConsumer = aSession.createConsumer(topic);
      MessageConsumer bConsumer = bSession.createConsumer(topic);
      MessageConsumer fConsumer = fSession.createConsumer(topic);
      ...
      Message bMsg = bConsumer.receiveNoWait();
      Assert.assertNotNull(bMsg);
      Assert.assertEquals("b,programmers", bMsg.getStringProperty("requiredRole"));
      Assert.assertNull(bConsumer.receiveNoWait());

      Message aMsg = aConsumer.receiveNoWait();
      Assert.assertNotNull(aMsg);
      Assert.assertEquals("a,programmers", aMsg.getStringProperty("requiredRole"));
      Assert.assertNull(aConsumer.receiveNoWait());

      Message fMsg1 = fConsumer.receiveNoWait();
      Message fMsg2 = fConsumer.receiveNoWait();
      Assert.assertNotNull(fMsg1);
      Assert.assertNotNull(fMsg2);
      Assert.assertEquals("a,programmers", fMsg1.getStringProperty("requiredRole"));
      Assert.assertEquals("b,programmers", fMsg2.getStringProperty("requiredRole"));
      Assert.assertNull(fConsumer.receiveNoWait());
   ...
   @Test
   public void testMessageAuthorizationTopicMultiNotAuthorized() throws Exception {
      JmsConnectionFactory factory = new JmsConnectionFactory("amqp://127.0.0.1:61616");
      Connection connection = factory.createConnection("first", "secret");
      Session session = connection.createSession(false, Session.AUTO_ACKNOWLEDGE);
      javax.jms.Topic topic = session.createTopic("TestTopicNotAuth");
